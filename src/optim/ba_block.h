...
class ProjectionFactor {
 public:
  ...
  inline bool Evaluate_Fac_huber(D const *const *const parameters, D *const residuals, D *const *const factors) const {
    const_map<quaternion> Q(parameters[0]);
    const_map<vector3> P(parameters[1]);
    const_map<vector3> pt_w(parameters[2]);
    map<vector3> res(residuals);
    ...
    factor_v4 << scale * a, huber_factor * huber_factor;
    factor_v5 << a, scale, huber_factor;
    return true;
  }
...
//     const vector3 a = (pt_w + P).normalized();
//     const D scale = 1.0f / (pt_w + P).norm();
//     res.noalias() = w*(a - Q.conjugate() * mea3d);
...
struct ParamBlock {
  ParamBlock(D *_ptr, int _size, LocalParameterization *_p) {
    ...
    for (size_t i = 0; i < size; ++i) param_new[i] = param_ptr[i];
  }
  ...
  void plus_delta(D *param_ptr_delta) {
    ...
    if (parameterization) {
      ...
      for (size_t i = 0; i < size; ++i) param_ptr_new[i] = param_ptr[i] + param_ptr_delta[i];
    }
  }

  void update() {
    for (size_t i = 0; i < size; ++i) param_ptr[i] = param_new[i];
  }

  double step_square_norm() {
    double sn = 0;
    for (size_t i = 0; i < size; ++i) sn += (param_new[i] - param_ptr[i]) * (param_new[i] - param_ptr[i]);
    return sn;
  }

  double x_square_norm() {
    double sn = 0;
    for (size_t i = 0; i < size; ++i) sn += param_ptr[i] * param_ptr[i];
    return sn;
  }
